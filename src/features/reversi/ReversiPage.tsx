...
import { BakuretsuReversiPage } from './BakuretsuReversiPage';
...
export function ReversiPage({ onBackToHome }: { onBackToHome: () => void }) {
  const [variant, setVariant] = useState<'normal' | 'bakuretsu' | null>(null);

  if (variant === 'normal') return <NormalReversiPage onBackToHome={onBackToHome} />;
  if (variant === 'bakuretsu') {
    return <BakuretsuReversiPage onBackToVariant={() => setVariant(null)} onBackToHome={onBackToHome} />;
  }
  return (
    <ReversiVariantSelectScreen
      onSelectNormal={() => setVariant('normal')}
      onSelectBakuretsu={() => setVariant('bakuretsu')}
      onBackToHome={onBackToHome}
    />
  );
}
